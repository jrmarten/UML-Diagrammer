...
public class LineDecorationChooser extends JFrame implements ActionListener {
	...
	private void addComponents() {
		...
		instructions = new JLabel("Select the " + (_isEnd ? "end" : "start")
				+ " decoration:");
		...
		items.add(new JLabel(createImageIcon(
				"/edu/uwm/cs361/images/compositionDecoration.png",
				"Composition")));
		items.add(_aggregation);
		items.add(new JLabel(createImageIcon(
				"/edu/uwm/cs361/images/aggregationDecoration.png",
				"Aggregation")));
		items.add(_association);
		items.add(new JLabel(createImageIcon(
				"/edu/uwm/cs361/images/arrowTipDecoration.png", "Arrow")));
		items.add(_none);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (ac.equals("ok") || ac.equals("cancel")) {
			if (ac.equals("ok")) {
				...
				if (_composition.isSelected())
					conType = ConnectionType.COMPOSITION;
				else if (_aggregation.isSelected())
					conType = ConnectionType.AGGREGATION;
				else if (_association.isSelected())
					conType = ConnectionType.ASSOCIATION;
				else
					conType = ConnectionType.DEPENDENCY;

				Connection con = _data.getData();

				con.setConnectionType((_isEnd) ? con.getEnd() : con.getStart(),
						conType);

				AttributeKey<LineDecoration> key = AttributeKeys.START_DECORATION;
				if (_isEnd)
					key = AttributeKeys.END_DECORATION;
			...
			if (ac.equals("cancel"))
				Util.dprint("You clicked cancel.");
			this.dispose();
		}
	}
